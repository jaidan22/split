...
const CardPanel = () => {
  ...
  return (
    <div className="cardpanel h-7-8 sm:mb-8 sm:mr-10 my-3 mx-2 bg-white rounded-lg ">
      <ListPopUp
        title="To Recieve"
        open={showLended}
        active={lended}
      ...
      <div className="summary flex sm:flex-row flex-col w-full items-center justify-evenly py-8 sm:mx-0 m-auto gap-4">
        <div
          className="lended hover:shadow-md flex flex-col justify-center w-56 h-48 bg-green-100 rounded-lg"
          onClick={() => showLended(true)}
        >
          <span className="title font-bold text-lg text-green-800">
            Total Lended
          </span>
          <span className="amt text-green-800 text-5xl mt-4">{lendedData.total}</span>
        </div>

        <div
          className="borrowed hover:shadow-md flex flex-col justify-center w-56 h-48 bg-red-200 rounded-lg"
          onClick={() => showBorrowed(true)}
        >
          <span className="title text-lg font-bold text-red-800">
            Total Borrowed
          </span>
          <span className="amt text-red-800 text-5xl mt-4">{borrowedData.total}</span>
        </div>
      </div>
    </div>
  );
